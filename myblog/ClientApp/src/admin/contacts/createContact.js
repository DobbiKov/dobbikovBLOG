...
export const AdminCreateContact = () => {
    ...
    const clickHandler = async () => {
        const response = await request('/api/Contacts/Create', 'POST', JSON.stringify({...form}), {"Accept": "application/json", "Content-Type" : "application/json"});
        const data = response.json();
        setCreated(true);
    }
    return(
        <div>
            {created == true ? <Redirect to="/admin/contacts"/> :
            <div>
                <p>Укажите название контакта</p>
                <input type="input" id="Name" name="Name" onChange={changeHandler}/>
                <p>Заполните текст контакта</p>
                <input type="input" id="Title" name="Title" onChange={changeHandler}/>
                <p>Укажите ссылку на контакт</p>
